...
def test_ap_forward():
    ...
    assert model_data[:,1] > 0
    assert model_data[:,2] > 0
...
def test_zirc_forward():
    ...
    dict = {'mineral type':['zircon', 'zircon', 'zircon'], 'age':[60,80,90],'error':[2,2,2],'size':[60,60,60],'U ppm':[100,500,1000],'Th ppm':[10,10,10],'Sm ppm':[0,0,0],'diffusion model':['guenthner','guenthner','guenthner'],'annealing model':['guenthner','guenthner','guenthner']}

    grain_in = pd.DataFrame(data=dict,
                        columns = ['mineral type', 'age', 'error','size','U ppm', 'Th ppm', 'Sm ppm', 'diffusion model', 'annealing model'],
                        index = ['sample 1', 'sample 2', 'sample 3'])
    
    obs_data = np.array([[62.8,1.2,250,80],[70.1,2.1,1500,85],[55.2,0.9,500,65]])
